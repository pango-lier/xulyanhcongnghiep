...
@section('content')
  <div class="mb-3">
            <a class="btn btn-success" href="admin/product/create">Thêm</a></div>
@if(session('success'))            
                    ...
                    @foreach($products as $row) 
                        <tr>
                            ...
                            <td style="text-align: center;">
                              ...
                              @else 
                                storage/product/default.png
                              @endif">
                              <span>{{$row->type}}</span>
                              {{--<a href="admin/post/comment/{{$row->id}}"  class="">({{$row->comment->count()}})</a>--}}
                            </td>
                            <td>
                              <a href="admin/product/edit/{{$row->id}}"  class="btn btn-sm btn-success">Sửa</a>
                              <a onclick="redirect_del('admin/product/destroy/{{$row->id}}')"  class="btn btn-sm btn-danger">Xóa</a>
                            </td>
                        </tr>
                    @endforeach
                    <script>
